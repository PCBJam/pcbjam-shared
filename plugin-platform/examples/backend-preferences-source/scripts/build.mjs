...
<title>Backend Preferences</title><style>${css}</style></head>
<body><div id="root"></div><script>${script}</script></body>
  ...
  const files = {
    ...
    'README.md': 'Backend preferences example. Configure and run its separate backend, then request endpoint approval from PCBJam before installing this plugin. Source is in the developer guide.\n',
  };
